...
**部署**：`deploy/` 是 `scripts/prepare-deploy.sh` 生成的快照——**只改 `app/` 和 `web/`，不要直接改 `deploy/`**。NAS 发布：`./scripts/deploy-nas.sh`。
...
- 版本号四处同步：`setup_app.py`、`web/package.json`、`app/main.py` APP_VERSION、CHANGELOG.md；bugfix 只升 rc。当前 0.8.0-rc1。
