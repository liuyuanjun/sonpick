...
> 最新版本：[![Latest Tag](https://img.shields.io/github/v/tag/liuyuanjun/sonpick)](https://github.com/liuyuanjun/sonpick/releases/latest)
...
- 搜索歌曲、导入文本列表或粘贴歌单链接批量下载，支持 QQ 音乐/网易云/咪咕/酷狗/酷我/千千 6 个源（来源可选、顺序可调；歌单链接解析后勾选曲目，按平台 song_id 锁定下载）
- 曲库扫描、浏览、整理、刮削信息、获取歌词、转码 MP3 和删除文件
- 本地与 WebDAV 多媒体源；同一逻辑歌曲可保留多个 FLAC、MP3 或远端版本
- WebDAV 套件上传：音频与可选封面、歌词一起上传，支持重命名、覆盖或跳过冲突
- 全局播放器、封面、同步歌词、播放队列、收藏、歌单、艺术家、专辑和播放历史；随机播放全部使用轻量播放池与前端洗牌，随机轮次内不重复，上一曲支持播放历史回退
- 歌曲列表显示格式与大小（多版本标注 `+N`，取音质最高的可用版本），每行可打开「歌曲信息」弹窗查看封面、元信息与全部文件版本明细
- 播放器外壳分三层：系统侧边栏「我的音乐」六项进入 `/player/<section>` 列表页（列表独占整宽）、底部悬浮胶囊播放条、以及全局大播放器抽屉（桌面覆盖内容区并保留侧边栏，移动端全屏）。胶囊通过 Teleport 挂载到 `body`，避免 Safari 下被布局侧栏的层叠上下文遮盖
- 下载、扫描、转码、刮削信息和获取歌词任务中心，实时展示任务状态与操作日志
- 单用户密码登录、JWT 认证，以及明暗主题和移动端适配

### 歌词与元信息

「刮削信息」与「获取歌词」是两套独立能力：分别配置来源、测试、排序与启停，并使用独立 API、任务类型和操作日志。

- 刮削覆盖标题、艺术家、专辑、年份、风格和封面；采用前可逐项比较，封面会显示新旧图片、尺寸与旧文件大小（无旧封面时默认采用新封面，已有则默认保留）。保存后会把所选信息和封面写入同一歌曲的全部可用本地文件版本。
- 歌词支持同步歌词、纯文本和纯音乐标记；来源包括 LRCLIB（无需 Key，串行节流）、网易云与咪咕。保存或清空时会同步处理全部本地版本的内嵌歌词与 `.lrc` 侧车。
- 两个工作台都会列出全部本地和 WebDAV 歌曲文件；WebDAV 版本明确标记为远端只读，不在本地保存流程中修改。
- 空歌词不会覆盖已有内容；清空必须单独确认。单曲/批量任务锁定歌曲 ID，播放器仅在目标仍为当前曲时刷新界面。
- 曲库、播放器列表与来源页可发起批量获取歌词；任务中心展示匹配、写入、纯音乐、跳过、未命中、限流等待与失败统计。
